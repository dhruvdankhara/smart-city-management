...
export default function CitizenComplaintDetail() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setError(error.response?.data?.message || "Failed to cancel complaint");
    } finally {
  ...
  const canCancel = ["reported", "assigned"].includes(complaint.status);
        ...
        <div className="space-y-4">
          <div className="rounded-xl border bg-card p-5 space-y-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>{complaint.categoryId?.name}</span>
            </div>
          </div>
        </div>
      </div>
